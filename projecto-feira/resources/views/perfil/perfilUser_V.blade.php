  ...
  <aside class="main-sidebar sidebar-dark-primary elevation-4">
    ...
    <div class="sidebar">
      ...
      <div class="user-panel mt-3 pb-3 mb-3 d-flex">
        <div class="image">
          <img src="/storage/fotos/VENDEDOR/{{$vendedor->user->foto}}" class="img-circle elevation-3" alt="{{$vendedor->user->name}}" style="width: 80px; he">
        </div>
        <div class="info">
          <!-- <a href="#" class="d-block">{{$vendedor->user->name}}</a> -->
              ...
              <div class="card-body box-profile">
                <div class="text-center">
                  <img class="profile-user-img img-fluid img-circle"
                       src="/storage/fotos/vendedor/{{$vendedor->user->foto}}"
                       alt="{{$vendedor->user->name}}">
                </div>

                <h3 class="profile-username text-center">{{$vendedor->user->name}}</h3>

                <p class="text-muted text-center">{{$vendedor->user->tipo}}</p>

                <ul class="list-group list-group-unbordered mb-3">
                  <li class="list-group-item">
                    <b>Feiras inscrita</b> <a class="float-right">{{count($vendedor->mercados)}}</a>
                  </li>
                  <li class="list-group-item">
                    <b>Visitas na Loja</b> <a class="float-right">{{$vendedor->visitas}}</a>
                  </li>
                  <!---li class="list-group-item">
              ...
              <div class="card-body">
                ...
                <!---------------------- DADOS DO VENDEDOR ----------------------->
                  <div class="active tab-pane" id="activity">
                    ...
                    <div class="post">
                      <div class="form-group row">
                        <div class="col-md-6">
                          <label>Nome:</label> <p>{{$vendedor->user->name}}</p>
                          <label>E-mail: </label> <p>{{$vendedor->user->email}}</p>
                          <label>Província: </label><p> {{$vendedor->provincias($vendedor->id)}}</p>
                        </div>
                        <div class="col-md-6">
                          <label>Telefone:</label> <p>{{$vendedor->telefone}}</p>
                          <label>Data de inscrição: </label><p> {{$vendedor->created_at}}</p>
                          <label>Categoria Produtos: </label> <p>{{$vendedor->produtos}}</p>
                        </div>
                      </div>
                    </div>
                    <!-- /.post -->
                  ...
                  <div class="tab-pane" id="timeline">
                      ...
                      <div class="row">
                        @forelse($vendedor->mercados as $mercado)
                          <div class="col-md-6 col-lg-4">
                      
                            <a href="{{route('todos.vendedores', [$mercado->id])}}"  alt="{{$mercado->designacao}}" class="media-1">
                              <img src="/fotos/mercado/{{$mercado->cartaz_mercado}}" alt="{{$mercado->designacao}}" class="img-fluid" style = "width: 550px; height:250px;">
                              <div class="media-1-content">
                                ...
                                <span class="category">
                                  <button href="{{route('inscrever.mercado', [$mercado->id])}}" class="btn btn-primary btn-md" style="background-color: #f3ae34; border: red;">Visitar</button>
                                </span><br>
                              </div>
                            </a>
                          </div>
                        @empty
                          <p>Não estás inscrito em nenhum mercado, por favor inscreva-se</p>
                        @endforelse
                      </div>
                  ...
                  <div class="tab-pane" id="settings">
                    <form class="form-horizontal" method="POST" action="{{route('editarPerfil.vendedor')}}">
                    @csrf
                    @method('PUT') 
                      <div class="row">
                        ...
                        <div class="col-sm-6">
                          ...
                          <label for="inputName" class="col-sm-5 col-form-label">Name</label>
                            <input type="text" class="form-control" id="inputName" name="name" value="{{$vendedor->user->name}}">
                          </div>
                        </div>
                        ...
                        <div class="col-sm-6">
                          <div class="form-group">
                            <label for="inputName" class="col-sm-5 col-form-label">Telefone</label>
                              <input type="text" class="form-control" id="inputName" name="telefone" value="{{$vendedor->telefone}}">
                          </div>
                        </div>
                      </div>
                          ...
                          <div class="col-sm-6">
                            <div class="form-group">
                              <label for="inputEmail" class="col-sm-5 col-form-label">Email</label>
                                <input type="email" class="form-control" id="inputEmail" name="email" value="{{$vendedor->user->email}}">
                            </div>
                          </div>
                          <div class="col-sm-6">
                            <div class="form-group">
                              <label for="preco" class="col-sm-5 col-form-label">Preco</label>
                                <input type="file" class="form-control" id="preco" name="preco" value="{{$vendedor->preco}}">
                            </div>
                          </div>
                      </div>
